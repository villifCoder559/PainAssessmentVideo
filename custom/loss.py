...
class CDW_CELoss(nn.Module):
  ...
  Repo: https://github.com/carlomarxdk/cdw-cross-entropy-loss
  """
  ...
  def forward(self, logits: Tensor, target: Tensor) -> Tensor:
    ...
    """
    if self.w.device != logits.device:
      self.w = self.w.to(logits.device)
    logits = torch.nn.functional.softmax(logits, dim=-1)  # apply softmax to logits
