...
Both upstream scripts (cross_space_projection.py and cross_space_logs.py) are
imported as modules and left unchanged. Stage outputs are routed into separate
subfolders via os.chdir() because run_optuna / cross_space_projection both
derive their output root from os.getcwd().
"""
import argparse
import itertools
import os
...
def _chdir(target):
  """
  Temporarily change cwd to `target`, restoring on exit.

  Args:
    target (str): Directory to chdir into.
  """
  prev = os.getcwd()
  os.chdir(target)
  try:
...
def _run_stage_val(args, stage_val_dir, valid):
  ...
  The output root is routed under stage_val_dir by chdir'ing there before the
  call, because run_optuna builds its path as cwd/Cross_projection/search_*.
  ...
  with _chdir(stage_val_dir):
    return csp.run_optuna(ns)
...
def _run_stage_test(args, top_k_df, stage_test_dir):
  ...
  with _chdir(stage_test_dir):
    for i, row in top_k_df.iterrows():
      ...
      pkls.append(csp.cross_space_projection(ns))
  return pkls
